...
BEGIN
  ...
  -- 2) Welcome email to the new user (role-specific, RO)
  IF v_service_key IS NOT NULL AND v_profile.email IS NOT NULL AND v_profile.email <> '' THEN
    IF v_role IN ('PROVIDER_ADMIN','PROVIDER_STAFF') THEN
    ...
    v_first_name := split_part(COALESCE(NULLIF(v_profile.full_name,''), ''), ' ', 1);

    IF v_welcome_tpl IS NOT NULL THEN
      BEGIN
        PERFORM net.http_post(
          url := v_supabase_url || '/functions/v1/send-transactional-email',
          headers := jsonb_build_object('Content-Type','application/json','Authorization','Bearer ' || v_service_key),
          body := jsonb_build_object(
            'templateName', v_welcome_tpl,
            'recipientEmail', LOWER(v_profile.email),
            'idempotencyKey', 'welcome-' || v_profile.id::text,
            'templateData', jsonb_build_object(
              'firstName', NULLIF(v_first_name,''),
              'tenantName', v_tenant_name,
              'dashboardUrl',
                CASE WHEN v_welcome_tpl = 'welcome-provider'
                     THEN 'https://greengrasscrm.ro/provider'
                     ELSE 'https://greengrasscrm.ro/client'
                END
            )
          )
        );
