...
BEGIN
  ...
  -- Read service-role key (populated by email infra setup)
  BEGIN
  ...
  -- 2) Bell notification + 3) email send per super admin
  FOR v_sa IN
  ...
  LOOP
    ...
    IF v_recipient_email <> '' AND v_service_key IS NOT NULL THEN
      BEGIN
        PERFORM net.http_post(
          ...
          body := jsonb_build_object(
            ...
            'templateData',   jsonb_build_object(
              ...
              'fullName',      COALESCE(NULLIF(v_profile.full_name,''), 'Necunoscut'),
              'email',         v_profile.email,
              'role',          v_role,
              'label',         v_label,
