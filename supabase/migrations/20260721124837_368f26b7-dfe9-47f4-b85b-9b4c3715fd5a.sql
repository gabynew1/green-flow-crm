CREATE OR REPLACE FUNCTION public.lifecycle_drip_candidates(_step text, _safety_cap integer DEFAULT 200)
 RETURNS TABLE(user_id uuid, tenant_id uuid, email text, first_name text, email_verified boolean, tenant_paused boolean, cat_onboarding_enabled boolean, customers_count integer, visits_count integer, offers_count integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  _window_clause text;
BEGIN
  -- Only callable by service_role (no-op guard; permissions are enforced by GRANT/REVOKE below)
