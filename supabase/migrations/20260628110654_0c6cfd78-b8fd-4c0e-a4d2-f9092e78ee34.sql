...
-- Helper: initialize a brand-new provider tenant (trial + seed defaults). Idempotent.
...
BEGIN
  ...
  SELECT status, trial_expires_at INTO v_status, v_trial
  FROM public.tenants WHERE id = p_tenant_id;

  -- Set trial status if not already in a paid/locked state
  IF COALESCE(v_status,'') NOT IN ('trial','professional','enterprise','locked','suspended') THEN
    UPDATE public.tenants
       SET status = 'trial',
           trial_expires_at = COALESCE(trial_expires_at, now() + interval '14 days')
     WHERE id = p_tenant_id;
  ELSIF v_trial IS NULL AND v_status = 'trial' THEN
    UPDATE public.tenants SET trial_expires_at = now() + interval '14 days' WHERE id = p_tenant_id;
  END IF;

  -- Seed service catalog from global defaults if tenant has none
  ...
  -- Seed a default service zone if none exists
  SELECT EXISTS(SELECT 1 FROM public.service_zones WHERE tenant_id = p_tenant_id) INTO v_has_zone;
  IF NOT v_has_zone THEN
    INSERT INTO public.service_zones (tenant_id, name, color)
    VALUES (p_tenant_id, 'Zonă implicită', '#10b981');
  END IF;
END;
$$;
...
-- Extend signup fan-out: provider auto-init + client→provider-admin notifications
...
BEGIN
  ...
  v_is_provider := v_role IN ('PROVIDER_ADMIN','PROVIDER_STAFF');
  v_is_client   := v_role = 'CLIENT_USER';
  ...
  -- Auto-init provider tenant (trial + default seed)
  IF v_is_provider AND v_profile.tenant_id IS NOT NULL THEN
    BEGIN
      PERFORM public.fn_init_provider_tenant(v_profile.tenant_id);
  ...
  IF v_service_key IS NOT NULL AND NOT (v_extra_email = ANY(v_emailed)) THEN
    ...
    EXCEPTION WHEN OTHERS THEN NULL;
    END;
    v_emailed := array_append(v_emailed, v_extra_email);
  END IF;
  ...
  -- 4) Client signup → notify linked tenant's PROVIDER_ADMINs (bell + email)
  IF v_is_client AND v_profile.tenant_id IS NOT NULL THEN
    FOR v_pa IN
    ...
    LOOP
      BEGIN
        INSERT INTO public.user_notifications (user_id, kind, title, body, entity_type, entity_id)
        VALUES (
          v_pa.user_id, 'new_signup',
          'Client nou: ' || COALESCE(NULLIF(v_profile.full_name,''), v_profile.email, 'client'),
          ...
          PERFORM net.http_post(
            ...
            body := jsonb_build_object(
              ...
              'data', jsonb_build_object(
                ...
                'full_name',  COALESCE(v_profile.full_name,'(fără nume)'),
                'email',      v_profile.email,
                'role',       v_role,
              ...
              'idempotency_key','new-client-signup-' || v_profile.id::text || '-' || v_recipient_email
            )
          );
        EXCEPTION WHEN OTHERS THEN NULL;
        END;
