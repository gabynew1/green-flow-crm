...
const VisitReportEmail = ({ propertyName, providerName, performedDate, summary, reviewUrl }: VisitReportProps) => (
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>Service visit report for {propertyName || 'your property'}</Preview>
    <Body style={main}>
      <Container style={container}>
        <Text style={eyebrow}>{SITE_NAME}</Text>
        <Heading style={h1}>Service Visit Report</Heading>
        <Text style={text}>
          {providerName ? `${providerName} has` : 'Your provider has'} completed a service visit and sent you a report.
        </Text>
        <Section style={panel}>
...
export const template = {
  ...
  subject: (data: Record<string, any>) => `Visit report: ${data.propertyName || 'Your property'}`,
  displayName: 'Visit report sent to client',
  previewData: { propertyName: 'Vila Rozelor', providerName: 'GreenScape SRL', performedDate: '2026-04-10', summary: 'Lawn mowed, hedges trimmed, flower beds weeded.' },
...
const summaryPanel = { margin: '0 0 24px', padding: '14px 16px', backgroundColor: 'hsl(43, 90%, 97%)', borderRadius: '12px', border: '1px solid hsl(43, 60%, 85%)' }
const panelText = { margin: '0 0 4px', color: 'hsl(160, 30%, 15%)', fontSize: '15px', fontWeight: '600' as const }
const panelDetail = { margin: '0', color: 'hsl(160, 10%, 45%)', fontSize: '13px', lineHeight: '1.6' }
