...
const OfferSentEmail = ({ offerName, propertyName, providerName, totalValue, validUntil, reviewUrl }: OfferSentProps) => (
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>New offer ready for your review</Preview>
    <Body style={main}>
      <Container style={container}>
        <Text style={eyebrow}>{SITE_NAME}</Text>
        <Heading style={h1}>New Offer for You</Heading>
        <Text style={text}>
          {providerName ? `${providerName} has` : 'Your provider has'} sent you a new offer.
        </Text>
        <Section style={panel}>
          {offerName && <Text style={panelText}>📋 {offerName}</Text>}
          {propertyName && <Text style={panelDetail}>Property: {propertyName}</Text>}
          {totalValue != null && <Text style={panelDetail}>Total value: {totalValue.toLocaleString('ro-RO')} RON</Text>}
          {validUntil && <Text style={panelDetail}>Valid until: {validUntil}</Text>}
        </Section>
        <Text style={text}>Please review the offer and accept or reject it.</Text>
        {reviewUrl && (
          <Button style={button} href={reviewUrl}>Review Offer</Button>
        )}
...
export const template = {
  component: OfferSentEmail,
  subject: (data: Record<string, any>) => data.offerName ? `Offer: ${data.offerName}` : 'New offer for your review',
  displayName: 'Offer sent to client',
  previewData: { offerName: 'Amenajare Grădină', propertyName: 'Vila Rozelor', providerName: 'GreenScape SRL', totalValue: 4500 },
